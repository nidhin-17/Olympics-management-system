...
Optionally create an empty data file first (the app will create it for you if you skip this):
bash
...
bash
   python main.py
Pick an option from the menu (1–6). After each action it'll ask if you want to continue — type yes to keep going, anything else exits.
How to test it

No test framework needed, just walk through it manually:

Add two or three players from different countries with different medal counts.
Search for one of the IDs you just added and check the record comes back right.
Update a player's medals and confirm the points recalculate correctly.
Check the top players view — should be sorted highest to lowest.
Check the top countries view — medals and points should be aggregated properly per country.
Remove a player, then confirm they're gone from search and both leaderboards.
Enter an invalid menu option (like 9) — it should show an error and bring the menu back up, not crash.
Try searching or removing a player ID that doesn't exist — it should say so cleanly instead of throwing an error.
Docs
